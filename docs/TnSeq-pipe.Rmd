...
```{r, message = FALSE}
# get list of pool files
pool_files <- list.files(path = "../data/pool", 
  pattern = "_R1_", full.names = TRUE)

# import data
df_pool <- lapply(pool_files, function(x) {
  read_tsv(x) %>%
  filter(!is.na(pos))
}) %>% bind_rows()

# summarize multiple rows of same barcode to one unique observation per row
df_pool <- df_pool %>% group_by(barcode, rcbarcode) %>%
  summarize(
    nTot = sum(nTot), n = sum(n),
    across(matches("scaffold|strand|pos|n2"), function(x) {x[1]}),
    nPastEnd = sum(nPastEnd)
  )  %>% ungroup
...
The next step before proceeding with gene mapping is to deal with barcodes that map to two different locations. Since some bacteria have multiple copies of the same operon with almost identical DNA sequence, it is good to keep ambiguous Tn insertions instead of removing them. How Transposon mutants that map to two different locations are handled depends on subsequent steps of the BarSeq protocol. The following optional transformation moves alternative, secondary mapping locations to the primary column.
...
df_pool_annotated <- df_pool_annotated %>%
  mutate(
    ...
    central = between(pos_relative, 0.1, 0.9, NAbounds = NA)
  )
```
...
```{r}
# save to disk
write_tsv(df_pool_annotated, file = "../data/pool/H16_pool_combined.tsv")
```
